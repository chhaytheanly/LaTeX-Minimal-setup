...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        ...
        \caption{OSB background with washers}
        \label{fig:aug_osb_washers}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth,height=3.8cm,keepaspectratio]{figures/chapter3/augmentations/aug_8.jpg}
        \caption{Wood background with screws}
        \label{fig:aug_wood_screws}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        ...
        \caption{White-background washer scene}
        \label{fig:aug_white_background}
    \end{subfigure}
    
    \caption{Representative samples from the GEDDS training set after applying the configured geometric and photometric augmentations, including rotation, HSV variation, brightness changes, and grayscale effects.}
...
\textbf{RT-DETR (Baseline)} & \textbf{32} & \textbf{110} & \textbf{0.99} & \textbf{0.8} & $\mathbf{640 \times 640}$ \\ \bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:yolo11ged_arch} presents the complete YOLO11-GED architecture, illustrating how the EMAc2f, GhostBottleneck, and DySample components are strategically integrated throughout the backbone and neck.
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
        ...
        \tikzset{
            ...
            concat/.style={circle, draw, fill=gray!30, minimum size=0.6cm,
                inner sep=0pt, font=\small\bfseries, drop shadow},
            detect/.style={block, fill=yellow!15, minimum height=1.2cm},
        ...
        \draw[arrow] (sppf9.east) -- ++(1,0) |- (dysample10.west);
        ...
        \draw[arrow] (c2f15.east) -- ++(1,0) |- (conv16.west);
        \draw[arrow] (conv16.south) -- (concat17.north);
        \draw[arrow] (emac2f12.east) -- ++(1.5,0) |- (concat17.west);
        \draw[arrow] (concat17.south) -- (emac2f18.north);
        \draw[arrow] (emac2f18.south) -- (conv19.north);
        \draw[arrow] (conv19.south) -- (concat20.north);
        \draw[arrow] (sppf9.south) -- ++(0,-0.6) -| (concat20.west); 
        \draw[arrow] (concat20.south) -- (emac2f21.north);

        \node[detect] (detect) at (12,-6.75) {Detect \\ $n_c=80$};
        
        \draw[arrow] (c2f15.north) -- ++(0,0.8) -| (detect.north);
        \draw[arrow] (emac2f18.east) -- ++(1.5,0) |- (detect.west);
        \draw[arrow] (emac2f21.south) -- ++(0,-0.8) -| (detect.south);

        \begin{scope}[on background layer]
            \node[draw, dashed, rounded corners, fill=gray!5, fit=(conv0)(sppf9)(p3)(p4)(p5),
                inner sep=12pt, label={[font=\small\bfseries]above:Backbone}] {};
            \node[draw, dashed, rounded corners, fill=gray!5,
                fit=(dysample10)(c2f15)(upsample13)(emac2f12)(concat11)(concat14),
                inner sep=12pt, label={[font=\small\bfseries]above:Neck (FPAN)}] {};
            \node[draw, dashed, rounded corners, fill=gray!5,
                fit=(conv16)(emac2f21)(concat17)(concat20)(conv19)(emac2f18),
                inner sep=12pt, label={[font=\small\bfseries]above:Neck (PAN)}] {};
            \node[draw, dashed, rounded corners, fill=gray!5, fit=(detect), inner sep=15pt,
                label={[font=\small\bfseries]above:Head}] {};
        \end{scope}
    \end{tikzpicture}%
    }
...
\subsubsection{The EMAc2f Module}
\label{subsubsec:emac2f}
...
% TODO: Insert EMA attention map visualization
\begin{figure}[htbp]
    \centering
...
% TODO: Insert DySample visualization
\begin{figure}[htbp]
    \centering
